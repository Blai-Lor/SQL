...
SELECT ESPECIALITAT, AVG(SALARI_MENSUAL) AS SALARI_PROMIG
FROM INVESTIGADOR
GROUP BY ESPECIALITAT
ORDER BY SALARI_PROMIG ASC;
...
SELECT i.DNI, 
       (i.NOM || ' ' || i.LLINATGE1 || ' ' || i.LLINATGE2) AS NOM_COMPLET,
       p.REFERENCIA, 
       p.DESCRIPCIO, 
       (pa.PERCENTATGE / 100) AS PROPORCIO
FROM INVESTIGADOR i
JOIN PARTICIPACIO pa ON i.DNI = pa.DNI_INVESTIGADOR
JOIN PROJECTE p ON pa.CODI_PROJECTE = p.CODI
ORDER BY PROPORCIO DESC, NOM_COMPLET ASC;
...
SELECT DISTINCT i.NOM, i.LLINATGE1, i.LLINATGE2,
    CASE 
        WHEN i.ESPECIALITAT IN ('Física', 'Biologia') THEN 'Ciències'
        WHEN i.ESPECIALITAT IN ('Medicina', 'Infermeria') THEN 'Salut'
        WHEN i.ESPECIALITAT = 'Informàtica' THEN 'Tecnologia'
    END AS AREA_ESPECIALITZACIO
FROM INVESTIGADOR i
JOIN PARTICIPACIO pa ON i.DNI = pa.DNI_INVESTIGADOR
JOIN FACTURA f ON pa.CODI_PROJECTE = f.CODI_PROJECTE;
...
SELECT 'Investigador' AS TIPUS, DNI AS IDENTIFICADOR, (SALARI_MENSUAL * 12) AS COST
FROM INVESTIGADOR i
WHERE DNI IN (SELECT DNI_INVESTIGADOR FROM PARTICIPACIO)
