...
CREATE TABLE VENDA(
    DNI_CLIENT VARCHAR2(9) generated as identity primary key,
    ID_LLIBRE NUMBER,
...
-- Enunciado
-- 1. Crea una taula VENDA amb la següent informació: DNI client, ID llibre (FK a LLIBRE), data venda (TIMESTAMP) i preu.
-- 2. Agafa un llibre qualsevol (el seu ID ha d’estar dins una variable) iitera un nombre de vegades igual al nombre d’exemplars del llibre. 
-- Per cada iteració:
    -- 1. Redueix en 1 el nombre d’exemplars del llibre (a la taula, UPDATE).
    -- 2. Inserta un registre a la taula VENDA amb el DNI d’un client predefinit
    -- (es pot guardar a una constant), l’ID del llibre, el timestamp actual
    -- (SYSTIMESTAMP) i un preu qualsevol.

DECLARE
    vIdLlibre LLIBRE.ID%TYPE := 1;
    vExemplars LLIBRE.EXEMPLARS%TYPE;
    vDni CONSTANT VARCHAR2(9) := '12345678A';
BEGIN
    SELECT exemplars
    INTO vExemplars
    FROM LLIBRE
    WHERE id = vIdLlibre;
    FOR i IN 1..vExemplars LOOP UPDATE LLIBRE SET exemplars = exemplars - 1 WHERE id = vIdLlibre; 
        INSERT INTO VENDA VALUES (vDni, vIdLlibre, SYSTIMESTAMP, 20);
    END LOOP;
END;