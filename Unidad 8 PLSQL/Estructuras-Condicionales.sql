...
-- Enunciado
-- Fes un bloc anònim que, combinant IF i CASE:
-- 1. Agafi el JOB_TITLE d’un empleat concret, l’ID del qual s’ha de passar a través d’una variable.
-- 2. Si aquest JOB_TITLE no és ‘Programmer’, imprimeix en pantalla un missatge indicant que no és programador.
-- 3. Si aquest JOB_TITLE és ‘Programmer’, agafa la data de contractació del treballador i comprova el seu valor. Imprimeix el trimestre de
-- contractació segons aquesta data: 1 (De Gener a Març), 2 (D’Abril a Juny), 3 (De Juliol a Setembre), 4 (D’Octubre a Desembre).

DECLARE 
    vEmployeeID CONSTANT NUMBER := 44;
    vJOB_TITLE EMPLOYEES.JOB_TITLE%TYPE;
    vHireDate EMPLOYEES.HIRE_DATE%TYPE;
BEGIN
    SELECT JOB_TITLE, HIRE_DATE 
    INTO vJOB_TITLE, vHireDate 
    FROM EMPLOYEES 
    WHERE EMPLOYEE_ID = vEmployeeID;

    IF vJOB_TITLE != 'Programmer' THEN
        DBMS_OUTPUT.PUT_LINE('No es programador');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Es programador, contratado en:');
        CASE 
            WHEN EXTRACT(MONTH FROM vHireDate) IN (1,2,3) THEN
                DBMS_OUTPUT.PUT_LINE('1r Trimestre');
            WHEN EXTRACT(MONTH FROM vHireDate) IN (4,5,6) THEN
                DBMS_OUTPUT.PUT_LINE('2n Trimestre');
            WHEN EXTRACT(MONTH FROM vHireDate) IN (7,8,9) THEN
                DBMS_OUTPUT.PUT_LINE('3r Trimestre');
            ELSE
                DBMS_OUTPUT.PUT_LINE('4t Trimestre');
        END CASE;
    END IF;
END;