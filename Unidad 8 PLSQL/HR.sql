...
DECLARE
  TYPE t_array IS VARRAY(3) OF VARCHAR2(50);
  v_arr t_array := t_array();
BEGIN
  SELECT first_name BULK COLLECT INTO v_arr FROM ( 
    SELECT emp.first_name FROM employees emp JOIN orders ord ON ord.salesman_id = emp.employee_id
    WHERE ord.status = 'Shipped' GROUP BY emp.first_name ORDER BY COUNT(*) DESC) WHERE ROWNUM <= 3;

  DBMS_OUTPUT.PUT_LINE('Top 3 empleats:');
  FOR i IN 1 .. v_arr.COUNT LOOP
    DBMS_OUTPUT.PUT_LINE(v_arr(i));
  END LOOP;
  v_arr.TRIM(2);
  DBMS_OUTPUT.PUT_LINE('Elements restants: ' || v_arr.COUNT);
  IF v_arr.COUNT = 1 THEN
    DBMS_OUTPUT.PUT_LINE('Empleat restant: ' || v_arr(1));
  END IF;
END;
