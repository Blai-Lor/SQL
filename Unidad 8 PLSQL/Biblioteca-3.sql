...
DECLARE
    vLlibres LLIBRE_VARRAY := LLIBRE_VARRAY();
BEGIN
    SELECT * BULK COLLECT INTO vLlibres FROM LLIBRE ORDER BY EXEMPLARS DESC FETCH FIRST 3 ROWS ONLY;
    FOR i IN 1..vLlibres.COUNT LOOP
        DBMS_OUTPUT.PUT_LINE('Llibre ' || i || ': ' || vLlibres(i).titol || ' - Autor: ' || (SELECT nom || ' ' || cognoms FROM AUTOR WHERE ID = (SELECT ID_autor FROM AUTOR_LLIBRE WHERE ID_llibre = vLlibres(i).ID)));
    END LOOP;
END;