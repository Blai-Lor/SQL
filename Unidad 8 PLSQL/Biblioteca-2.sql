...
DECLARE
    vIdLlibre CONSTANT NUMBER := 13;
    vTitol LLIBRE.TITOL%TYPE;
    vIdSequela LLIBRE.ID_SEQUELA_DE%TYPE;
    vTitolOriginal LLIBRE.TITOL%TYPE;
BEGIN
    SELECT TITOL, ID_SEQUELA_DE INTO vTitol, vIdSequela 
    FROM LLIBRE WHERE ID = vIdLlibre;
    IF vIdSequela IS NULL THEN
        DBMS_OUTPUT.PUT_LINE('El libro ' || vTitol || ' no es secuela de ningun otro libro.');
    ELSE
        SELECT TITOL INTO vTitolOriginal FROM LLIBRE WHERE ID = vIdSequela;
        DBMS_OUTPUT.PUT_LINE('El libro ' || vTitol || ' es secuela de ' || vTitolOriginal || '.');
    END IF;
END;
...
DECLARE
    vIdLlibre CONSTANT NUMBER := 9;
    vCantAfegir CONSTANT NUMBER := 4;
    vExemplarsActuals NUMBER;
BEGIN
    SELECT EXEMPLARS INTO vExemplarsActuals FROM LLIBRE WHERE ID = vIdLlibre;
    DBMS_OUTPUT.PUT_LINE('El libro ' || vIdLlibre || ' tiene ' || vExemplarsActuals || ' ejemplares. Se ha pedido añadirle ' || vCantAfegir || ' ejemplares.');
    FOR i IN 1 .. vCantAfegir LOOP
        vExemplarsActuals := vExemplarsActuals + 1;
        UPDATE LLIBRE SET EXEMPLARS = vExemplarsActuals WHERE ID = vIdLlibre;
        DBMS_OUTPUT.PUT_LINE('Se ha añadido. Ejemplares: ' || vExemplarsActuals);
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('Ejemplares actuales: ' || vExemplarsActuals);
END;
...
DECLARE
    vIdLlibre CONSTANT NUMBER := 7;
    vCantEliminar CONSTANT NUMBER := 5;
    vExemplarsActuals NUMBER;
    vNoEliminats NUMBER := 0;
BEGIN
    SELECT EXEMPLARS INTO vExemplarsActuals FROM LLIBRE WHERE ID = vIdLlibre;
    DBMS_OUTPUT.PUT_LINE('El libro con ID ' || vIdLlibre || ' tiene ' || vExemplarsActuals || ' ejemplares. Se ha solicitado eliminar ' || vCantEliminar || ' ejemplares.');
    FOR i IN 1 .. vCantEliminar LOOP
        IF vExemplarsActuals > 0 THEN
            vExemplarsActuals := vExemplarsActuals - 1;
            UPDATE LLIBRE SET EXEMPLARS = vExemplarsActuals WHERE ID = vIdLlibre;
            DBMS_OUTPUT.PUT_LINE('Ejemplar eliminado. Ejemplares: ' || vExemplarsActuals);
        ELSE 
            vNoEliminats := vCantEliminar - (i - 1);
            EXIT;
        END IF;
    END LOOP;
    IF vNoEliminats > 0 THEN
        DBMS_OUTPUT.PUT_LINE('No quedan mas ejemplares. ' || vNoEliminats || ' ejemplares no se han eliminado.');
    END IF;
    DBMS_OUTPUT.PUT_LINE('Ejemplares actuales: ' || vExemplarsActuals);
END;