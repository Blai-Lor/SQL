...
-- G.Skill ha llançat un nou kit de memòria com a part de la seva línia de
-- productes “G.Skill Ripways V Series” amb una velocitat millorada
-- (DDR4-3600) i preu (Standard cost: 697.56, list price: 779.99).

-- • Utilitza un record per obtenir dades d’un producte “G.Skill Ripjaws V
-- Series” existent i altera el seu preu i la seva descripció, indicant la
-- nova velocitat. Insereix aquestes noves dades a la taula PRODUCTS
-- com un nou registre.

DECLARE 
    vProducte PRODUCTS%ROWTYPE;
BEGIN
    SELECT * INTO vProducte 
    FROM PRODUCTS
    WHERE product_name='G.Skill || Rip'
    AND description LIKE '%DDR4-333%'
    AND rownum=1;

    vProducte.description := REPLACE(vProducte.description,'3333','3600');
    vProducte.list_price := 1234;
    vProducte.standard_cost := 4321;

    INSERT INTO PRODUCTS(product_id,product_name,description,standard_cost,list_price,category_id) 
    VALUES (vProducte.product_id, vProducte.product_name, vProducte.description, vProducte.standard_cost,
        vProducte.list_price, vProducte.category_id
    );
