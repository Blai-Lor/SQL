...
WHERE l.ID = lg.ID_llibre AND lg.nom_genere = g.nom;
...
WHERE l.ID = al.ID_llibre AND al.ID_autor = a.ID AND a.nacionalitat != 'ESP';
...
SELECT l.titol, lg.nom_genere, a.nom, a.cognoms
FROM LLIBRE l, LLIBRE_GENERE lg, AUTOR_LLIBRE al, AUTOR a
WHERE l.ID = lg.ID_llibre AND l.ID = al.ID_llibre AND al.ID_autor = a.ID;
...
SELECT a.nom, a.cognoms, SUM(l.exemplars)
FROM AUTOR a, AUTOR_LLIBRE al, LLIBRE l
WHERE a.ID = al.ID_autor AND al.ID_llibre = l.ID
...
SELECT l.titol, lg.nom_genere
FROM LLIBRE l
INNER JOIN LLIBRE_GENERE lg ON l.ID = lg.ID_llibre;
...
SELECT a.nom, a.cognoms, NVL(SUM(l.exemplars), 0) AS total_exemplars
FROM AUTOR a
LEFT JOIN AUTOR_LLIBRE al ON a.ID = al.ID_autor
...
SELECT a.nom, a.cognoms, MIN(l.an), MAX(l.an)
FROM AUTOR a
INNER JOIN AUTOR_LLIBRE al ON a.ID = al.ID_autor
INNER JOIN LLIBRE l ON al.ID_llibre = l.ID
GROUP BY a.nom, a.cognoms;