...
SELECT * FROM LLIBRE WHERE exemplars BETWEEN 5 AND 10;

-- Retorna el títol de cada llibre i el seu autor. Si no té autor, ha de sortir “Desconegut”.
SELECT LL.titol, NVL(AU.nom, 'Desconegut') AS autor FROM LLIBRE LL LEFT JOIN AUTOR_LLIBRE AULL ON AULL.ID_LLIBRE = LL.ID LEFT JOIN AUTOR AU ON AU.ID = AULL.ID_AUTOR;

-- Retorna el nom de cada autor i si és europeu (ESP, GBR) o no (USA, JPN).
SELECT autor.nom, CASE WHEN nacionalitat IN('ESP', 'GBR') THEN 'Europeu' ELSE 'No europeu' END AS ES_EUR FROM autor; 

-- Retorna els llibres llançats a l’any de naixement de qualsevol autor. Empra IN.
SELECT * FROM LLIBRE WHERE AN IN (SELECT EXTRACT(YEAR FROM data_naix) FROM AUTOR);