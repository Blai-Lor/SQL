...
SELECT titol || '-' || exemplar FROM LLIBRE;
SELECT nom || ' ' || cognoms FROM AUTOR;
SELECT SUBSTR(cognoms, 1, INSTR(cognoms, ' ')-1) || '-' || SUBSTR(cognoms, INSTR(cognoms, '-')+1) FROM AUTOR;
SELECT UPPER(REPLACE(cognoms, ' ', ' '), 1, 10) FROM AUTOR;
