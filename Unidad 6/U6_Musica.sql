...
SELECT ar.name, ar.biography, SUM(s.quantity) AS total_sales
FROM artists ar
...
SELECT ar.name, ar.country FROM artists ar
LEFT JOIN albums a ON ar.id_artist = a.artist 
WHERE a.artist IS NULL;

--11: Mostra el nom i la descripció dels gèneres que no tenen cap àlbum al catàleg de la botiga.
SELECT g.id_genre, g.name, g.description
FROM genres g
...
SELECT gen.name, NVL(SUM(sal.total),0) AS total_income
FROM genres gen
...
CASE WHEN year < 2000 THEN 'XX'
ELSE  'XXI' END FROM albums alb
