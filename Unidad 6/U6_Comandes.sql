...
SELECT DISTINCT cliente.id, nombre, apellido1 || ' ' || apellido2 AS apellido FROM cliente JOIN pedido ON cliente.id=pedido.id_cliente ORDER BY apellido, nombre;

-- 2. Devuelve un listado que solamente muestre los clientes que no han realizado ningún pedido.
SELECT cliente.* FROM cliente LEFT JOIN pedido ON pedido.id_cliente = cliente.id WHERE pedido.id IS NULL;

-- 3. Devuelve un listado que solamente muestre los comerciales que no han realizado ningún pedido.
SELECT comercial.* FROM comercial LEFT JOIN pedido ON pedido.id_comercial = comercial.id WHERE pedido.id IS NULL;
...
SELECT pedido.*,cliente.* FROM pedido JOIN cliente ON cliente.id=pedido.id_cliente ORDER BY cliente.nombre, cliente.apellido1, cliente.apellido2;
...
SELECT pedido.*,comercial.* FROM pedido JOIN comercial ON comercial.id=pedido.id_comercial ORDER BY comercial.nombre, comercial.apellido1, comercial.apellido2;
...
SELECT * FROM pedido JOIN cliente ON cliente.id=pedido.id_cliente JOIN comercial ON comercial.id=pedido.id_comercial;
...
SELECT 'cliente' AS tipo, id, nombre, apellido1, apellido2 FROM cliente UNION SELECT 'comercial' AS tipo, id, nombre, apellido1, apellido2 FROM comercial ORDER BY nombre, apellido1,apellido2;
...
SELECT cliente.* FROM cliente JOIN pedido ON pedido.id_cliente=cliente.id WHERE EXTRACT(YEAR FROM fecha)=2017 AND total>=300 AND total <=1000;
...
SELECT * FROM comercial co LEFT JOIN pedido pe ON pedido_comercial=co.id ORDER BY co.apellido1, co.apellido2, co.nombre; 
...
SELECT co.nombre, co.apellido1, co.apellido2 FROM comercial JOIN pedido ON pedido.id_comercial=comercial.id JOIN cliente ON cliente.id=pedido.id_cliente WHERE cliente.nombre ||' '|| cliente.apellido1 ||' '|| cliente.apellido2 = 'María Santana Moreno';

-- 11. Devuelve el nombre de todos los clientes que han realizado algún pedido con el comercial Daniel Sáez Vega.
SELECT cliente.nombre, cliente.apellido1, cliente.apellido2 FROM cliente JOIN pedido ON pedido.id_cliente=cliente.id JOIN comercial ON comercial.id=pedido.id_comercial WHERE comercial.nombre ||' '|| comercial.apellido1 ||' '|| comercial.apellido2 = 'Daniel Sáez Vega';
...
SELECT * FROM cliente cl LEFT JOIN pedido pe ON pedido.id_cliente=cl.id ORDER BY cl.apellido1, cl.apellido2, cl.nombre;