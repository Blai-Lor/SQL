...
SELECT lli.titol, edi.nom FROM LLIBRE, EDITORIAL WHERE lli.id_editorial=edi.id;
    -- Utilizando JOIN
SELECT lli.titol, edi.nom FROM LLIBRE lli JOIN EDITORIAL edi ON lli.id_editorial=edi.id;

-- Repeteix la consulta anterior excloent l’editorial Planeta.
SELECT lli.titol, edi.nom FROM LLIBRE, EDITORIAL WHERE lli.id_editorial=edi.id AND edi.nom!='Planeta';
    -- Utilizando JOIN
SELECT lli.titol, edi.nom FROM LLIBRE lli JOIN EDITORIAL edi ON lli.id_editorial=edi.id WHERE edi.nom!='Planeta';

-- Consulta els noms dels llibres i el nom del seu autor.
SELECT lli.titol, aut.nom FROM LLIBRE lli, AUTOR aut, AUTOR_LLIBRE aull WHERE aull.LLIBRE=lli.id AND aull.id_autor=aut.id;
    -- Utilizando JOIN
SELECT lli.titol, aut.nom FROM LLIBRE lli JOIN AUTOR_LLIBRE aull ON aull.id_llibre = lli.id JOIN AUTOR aut ON aull.id_autor = aut.id;

-- Repeteix la consulta anterior, però només mostrant aquells llibres d’un any anterior a 1950.
SELECT lli.titol, aut.nom FROM LLIBRE lli, AUTOR aut, AUTOR_LLIBRE aull WHERE aull.LLIBRE=lli.id AND aull.id_autor=aut.id AND lli.an<1950;
    -- Utilizando JOIN
SELECT lli.titol, aut.nom FROM LLIBRE lli JOIN AUTOR_LLIBRE aull ON aull.id_llibre = lli.id JOIN AUTOR aut ON aull.id_autor = aut.id WHERE lli.an<1950;

-- Llista els gèneres sense llibres.
SELECT gen.* FROM GENERE gen LEFT JOIN LLIBRE_GENERE llge ON llge.nom_genere=gen.nom WHERE llge.nom_genere IS NULL;
